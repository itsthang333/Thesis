...
class CandidateDiagnosticsTests(unittest.TestCase):
    def test_source_specific_features_round_trip_as_schema_four(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            masks = np.zeros((2, 8, 8), dtype=np.uint8)
            saved = save_candidate_diagnostics(
                root / "candidate_diagnostics" / "source.npz",
                sam_masks=masks,
                ...
                sam_scores=[0.7, 0.8], selection_scores=[0.1, 0.2],
                classifier_causal_scores=None, component_ids=[1, 2],
                prompt_modes=["point", "box"],
                proposal_source_ids=["top1", "top2"],
                source_map_mean_scores=[0.3, 0.4],
                source_map_mass_coverages=[0.5, 0.6],
                source_score_densities=[0.7, 0.8],
                source_class_ids=[1, 2],
                source_class_probabilities=[0.9, 0.8],
                source_class_ranks=[1, 2],
                cam_levels=[85.0, 90.0],
                prompt_ids=["a", "b"],
                multimask_indices=[0, 1],
            )
            summary = write_candidate_diagnostics_manifest(
                root,
                [{"image_name": "source.jpeg", **saved}],
                expected_image_names=["source.jpeg"],
                split="val", image_size=8,
                pseudo_manifest_sha256="pseudo-lock",
                selection_method="test", support_clip_kernel=0,
                cam_percentile=90.0, cohort="all",
            )
            indexed, _ = validate_candidate_diagnostics_manifest(
                root,
                expected_image_names=["source.jpeg"],
                split="val",
            ...
            self.assertEqual(set(indexed), {"source"})
            with np.load(root / "candidate_diagnostics" / "source.npz", allow_pickle=False) as payload:
                self.assertEqual(int(payload["schema_version"][0]), 4)
