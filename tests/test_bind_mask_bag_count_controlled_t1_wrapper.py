...
from audit_mask_bag_count_controlled_self_paced_t1_output import _verify_binding
from bind_mask_bag_count_controlled_t1_wrapper import (
    TEMPLATE_SHA256,
    bind,
    canonical_bytes,
    digest,
...
TEMPLATE = PROJECT / "kaggle_wrappers" / "run_mask_bag_count_controlled_t1_v1.py"
PROTOCOL = (
    ROOT
    / "artifacts"
    / "research_protocols"
...
def test_t1_wrapper_is_fail_closed_and_gt_blind() -> None:
    ...
    assert source.index("producer_gate_audit_sha256") < source.index("target_freeze_sha256")
    assert source.index("audit_output(source_hashes, cache, baseline, t4)") > source.index(
        "run_mask_bag_count_controlled_self_paced_t1_arm.py"
    )
    assert digest(canonical_bytes(TEMPLATE)) == TEMPLATE_SHA256


def test_t1_binder_changes_exactly_three_fields_and_matches_auditor(tmp_path: Path) -> None:
...
def test_t1_binder_refuses_invalid_version_or_existing_output(tmp_path: Path) -> None:
