...
from bind_mask_bag_critical_relation_r3_wrapper import bind, canonical_bytes, digest


ROOT = Path(__file__).resolve().parents[1]
TEMPLATE = ROOT / "project" / "kaggle_wrappers" / "run_mask_bag_critical_relation_r3_v1.py"


def test_r3_binder_exactly_replaces_three_external_fields(tmp_path: Path) -> None:
...
def test_r3_binder_refuses_existing_output(tmp_path: Path) -> None:
    output = tmp_path / "bound.py"
