...
def audit_b1_output(
    ...
    if gate.get("operational_gate_pass") is not True:
    ...
    for row in val_rows:
        ...
        if len(indices) > 1:
            correlation = float(np.corrcoef(bas_rank, upstream_rank)[0, 1])
            if np.isfinite(correlation):
                correlations.append(correlation)
        changes += int(int(np.argmax(transferred)) != int(np.argmax(three_way)))
        expected_scores = {
    ...
    if (
        ...
        or abs(mean_correlation - float(gate["mean_bas_upstream_rank_correlation"])) > 1e-12
        or abs(change_fraction - float(gate["three_way_changed_selection_fraction"])) > 1e-12
        or mean_correlation > 0.8
        or change_fraction < 0.05
    ):
        raise ValueError("B1 independent operational gate does not reproduce")
    return {
        "audit_id": "independent_bas_candidate_descriptor_b1_output_v1",
        "status": "PREDICTION_PAIR_PHYSICALLY_VERIFIED_GT_BLIND_GATE_PASS",
        ...
        "mean_bas_upstream_rank_correlation": mean_correlation,
        "three_way_changed_selection_fraction": change_fraction,
        "validation_gt_read": False,
        "consumer_trained": False,
        "test_evaluated": False,
    }
