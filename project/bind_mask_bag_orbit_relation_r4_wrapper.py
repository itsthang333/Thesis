...
"""Bind the finite R4 Kaggle wrapper exactly once before launch."""
...
TEMPLATE_SHA256 = "4924779ad3ce01cdd0c410f5dfbfdad78f1ba4c2650012f11503517df2f67a19"
PROTOCOL_PATH = "artifacts/research_protocols/rad_dino_mask_bag_orbit_relation_r4_v1.json"
...
def bind(
    ...
    *,
    ...
    if output.exists() or audit_output.exists():
        raise FileExistsError("R4 bound wrapper or launch binding already exists")
    ...
    if digest(template_payload) != TEMPLATE_SHA256:
        raise ValueError("R4 wrapper template SHA-256 mismatch")
    replacements = {
    ...
    for relative, expected in source_hashes.items():
        if digest(_git_bytes(repository_root, checkout_commit, relative)) != expected:
            raise ValueError(f"R4 source differs at execution checkout: {relative}")
    subprocess.run(["git", "merge-base", "--is-ancestor", SOURCE_COMMIT, checkout_commit], cwd=repository_root, check=True)
    ...
    audit_output.write_text(json.dumps(binding, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    if canonical_bytes(output) != bound or json.loads(audit_output.read_text(encoding="utf-8")) != binding:
        raise RuntimeError("R4 binding write/read verification failed")
    return binding
...
def main() -> None:
    ...
    result = bind(args.template.resolve(), args.output.resolve(), args.launch_binding.resolve(), repository_root=args.repository_root.resolve(), checkout_commit=args.checkout_commit, kernel_version=args.kernel_version)
