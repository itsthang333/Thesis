...
"""Bind the finite R3 Kaggle wrapper exactly once before launch."""
...
TEMPLATE_SHA256 = "c350238ad69231e6714c12d8c7cd8b7fc38b5d2a1702c22a55a0ed7ac6b6aa2e"
PROTOCOL_PATH = "artifacts/research_protocols/rad_dino_mask_bag_critical_relation_r3_v1.json"
...
def bind(
    ...
    *,
    ...
    if output.exists() or audit_output.exists():
        raise FileExistsError("R3 bound wrapper or launch binding already exists")
    ...
    if digest(template_payload) != TEMPLATE_SHA256:
        raise ValueError("R3 wrapper template SHA-256 mismatch")
    replacements = {
    ...
    for relative, expected in source_hashes.items():
        if digest(_git_bytes(repository_root, checkout_commit, relative)) != expected:
            raise ValueError(f"R3 source differs at execution checkout: {relative}")
    subprocess.run(
    ...
    if (
        ...
        raise RuntimeError("R3 binding write/read verification failed")
    return binding
