...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--profile", choices=("btxrd_best", "btxrd_hybrid"), default="btxrd_hybrid")
...
def main() -> None:
    ...
    if args.verify:
        ...
        recorded = document.pop("freeze_sha256", None)
        actual = hashlib.sha256(canonical_bytes(document)).hexdigest()
        if recorded != actual:
            raise ValueError(f"Frozen config checksum mismatch: recorded={recorded}, actual={actual}")
        print(json.dumps({"verified": True, "freeze_sha256": actual}, indent=2))
        return
