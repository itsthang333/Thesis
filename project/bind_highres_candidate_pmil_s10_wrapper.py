"""Bind the frozen S10 Kaggle wrapper exactly once before launch."""
...
TEMPLATE_SHA256 = "ae4b9eb751e48bcae3e1ce7d9a24669e923d8969a369b49aaf83f22f9bf403f9"
...
def bind(
    ...
    *,
    ...
    if output.exists() or binding_output.exists():
        raise FileExistsError("S10 bound wrapper or launch binding already exists")
    ...
    if digest(template_payload) != TEMPLATE_SHA256:
        raise ValueError("S10 wrapper template SHA-256 mismatch")
    if digest(_git_bytes(repository_root, checkout_commit, TEMPLATE_PATH)) != TEMPLATE_SHA256:
        raise ValueError("S10 wrapper template differs at execution checkout")
    replacements = {
    ...
    if (
        ...
        raise ValueError("S10 frozen protocol contract mismatch")
    subprocess.run(
    ...
    if canonical_bytes(output) != bound or json.loads(
        ...
        raise RuntimeError("S10 binding write/read verification failed")
    return binding
