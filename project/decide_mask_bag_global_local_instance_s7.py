...
"""Compare the frozen S7 pair and apply its predeclared post-GT gates."""
...
PAIR_FREEZE_SHA256 = "a3d37e43beb8e90d0d30fcf3a86c25bf071fcb340ec62a025ee429799f031a63"
IDENTITY_FREEZE_SHA256 = "f2a19a6b170554a000d82e1dafa127988e25533310193b1367a492ed53d0b2c9"
PRIMARY_FREEZE_SHA256 = "bae35d50b2f87bb969c3a0851c38270ab60ac081d85c1c2194fe370f70b77e1c"
IDENTITY_SCORE_MANIFEST_SHA256 = (
    "6d08db6c7fd29d5ed2bc7ff57133cd9fdea1adc65ec797f9c3f0ee424b3452fc"
)
PRIMARY_SCORE_MANIFEST_SHA256 = (
    "9f48985c48a118b6ce59f1c4ceb2ff4fc66a841bc537947a0d52ae622ad825de"
)
PRE_GT_STATUS = "PREDICTION_PAIR_PHYSICALLY_VERIFIED_GT_BLIND_PASS_LOCAL"
...
def _read_per_image(path: Path, expected_sha256: str) -> dict[str, dict[str, str]]:
    ...
    if len(rows) != 184 or not rows:
        raise ValueError("Each S7 evaluated arm must contain exactly 184 tumor rows")
    missing = sorted(REQUIRED_PER_IMAGE_FIELDS - set(rows[0]))
    if missing:
        raise ValueError(f"S7 per-image evaluation lacks fields: {missing}")
    indexed: dict[str, dict[str, str]] = {}
    for row in rows:
        image_id = row["image_id"]
        if not image_id or image_id in indexed:
            raise ValueError("S7 per-image evaluation has duplicate/empty image IDs")
        if row["size_group"] not in EXPECTED_COUNTS:
            raise ValueError("S7 per-image evaluation has an invalid subgroup")
        if row["complete_miss"] not in {"0", "1"} or row["baseline_complete_miss"] not in {"0", "1"}:
            raise ValueError("S7 per-image evaluation has an invalid miss flag")
        for field in (
            "gt_area_ratio",
            "dice",
            "oracle_best_single_dice",
    ...
    if counts != EXPECTED_COUNTS:
        raise ValueError(f"Frozen S7 subgroup counts differ: {counts}")
    return indexed
...
def _paired_group_bootstrap(
    ...
    *,
    ...
    if deltas.shape != (len(groups),):
        raise ValueError("S7 bootstrap values/groups do not align")
    ...
    return {
        "delta_primary_minus_identity": float(deltas.mean()),
        "ci95": [
...
def _verify_evaluation(
    ...
    *,
    ...
    _require_sha(expected_audit_sha256, name="evaluation audit SHA-256")
    if sha256_file(audit_path) != expected_audit_sha256:
        raise ValueError("S7 evaluation-audit SHA-256 mismatch")
    ...
    if (
        ...
        or audit.get("arm_prediction_freeze_sha256") != expected_arm_freeze_sha256
        or audit.get("candidate_score_manifest_sha256") != expected_score_manifest_sha256
        or audit.get("baseline_prediction_freeze_sha256") != BASELINE_FREEZE_SHA256
        ...
        or audit.get("complete_misses_included") is not True
        or audit.get("validation_gt_read_only_after_all_predictions_frozen_and_verified") is not True
    ):
        raise ValueError("S7 evaluation boundary/provenance mismatch")
    ...
    if (
        ...
        or summary.get("validation_gt_read_only_after_all_predictions_frozen_and_verified") is not True
        ...
        raise ValueError("S7 evaluator source/protocol/bootstrap mismatch")
...
def decide(
    ...
    protocol_path: Path,
    identity_evaluation_root: Path,
    identity_evaluation_audit_path: Path,
    primary_evaluation_root: Path,
    primary_evaluation_audit_path: Path,
    output_dir: Path,
    *,
    expected_pre_gt_audit_sha256: str,
    expected_identity_evaluation_audit_sha256: str,
    expected_primary_evaluation_audit_sha256: str,
) -> dict[str, Any]:
    _require_sha(expected_pre_gt_audit_sha256, name="pre-GT audit SHA-256")
    if sha256_file(protocol_path) != PROTOCOL_SHA256:
        raise ValueError("S7 protocol SHA-256 mismatch")
    protocol = _json(protocol_path)
    evaluation_protocol = protocol.get("post_freeze_evaluation", {})
    if (
        protocol.get("experiment_id") != EXPERIMENT_ID
        or evaluation_protocol.get("bootstrap_seed") != BOOTSTRAP_SEED
        or evaluation_protocol.get("operational_goals_overall_small_medium_large")
        != [GOALS[name] for name in SUBGROUPS]
    ):
        raise ValueError("S7 protocol evaluation contract mismatch")
    if sha256_file(pre_gt_audit_path) != expected_pre_gt_audit_sha256:
        raise ValueError("S7 pre-GT audit SHA-256 mismatch")
    ...
    if (
        ...
        or pre_gt.get("prediction_pair_freeze_sha256") != PAIR_FREEZE_SHA256
        or pre_gt.get("training_labels") != "binary_image_level_only"
        or pre_gt.get("evaluation_authorized_after_this_audit") is not True
        or pre_gt.get("validation_gt_read") is not False
    ):
        raise ValueError("S7 pre-GT audit did not authorize evaluation")
    _require_safety(pre_gt, name="pre-GT audit")

    identity, identity_audit = _verify_evaluation(
        identity_evaluation_root,
        identity_evaluation_audit_path,
        expected_audit_sha256=expected_identity_evaluation_audit_sha256,
        expected_arm_freeze_sha256=IDENTITY_FREEZE_SHA256,
        expected_score_manifest_sha256=IDENTITY_SCORE_MANIFEST_SHA256,
    )
    primary, primary_audit = _verify_evaluation(
        ...
        expected_arm_freeze_sha256=PRIMARY_FREEZE_SHA256,
        expected_score_manifest_sha256=PRIMARY_SCORE_MANIFEST_SHA256,
    )
    if set(identity) != set(primary):
        raise ValueError("S7 evaluated arm image identities differ")

    paired_rows: list[dict[str, Any]] = []
    identity_exactly_baseline = True
    for image_id in sorted(identity):
        old = identity[image_id]
        ...
        for field in (
            ...
            if old[field] != new[field]:
                raise ValueError(f"S7 frozen paired field differs for {image_id}: {field}")
        old_dice = float(old["dice"])
        new_dice = float(new["dice"])
        baseline_dice = float(old["baseline_dice"])
        old_miss = int(old["complete_miss"])
        new_miss = int(new["complete_miss"])
        identity_exactly_baseline &= old_dice == baseline_dice
        identity_exactly_baseline &= old_miss == int(old["baseline_complete_miss"])
        paired_rows.append(
            {
                "image_id": image_id,
    ...
    for subgroup_index, subgroup in enumerate(SUBGROUPS):
        ...
        primary_values = np.asarray([float(row["primary_dice"]) for row in rows])
        identity_values = np.asarray([float(row["identity_dice"]) for row in rows])
        metrics[subgroup] = {
            **_paired_group_bootstrap(
            ...
            "primary_dice": float(primary_values.mean()),
            "identity_dice": float(identity_values.mean()),
            "primary_complete_misses": int(sum(int(row["primary_complete_miss"]) for row in rows)),
            "identity_complete_misses": int(sum(int(row["identity_complete_miss"]) for row in rows)),
    ...
    mechanism_checks = {
        "overall_mean_strictly_improves": metrics["overall"]["delta_primary_minus_identity"] > 0.0,
        "small_mean_strictly_improves": metrics["small"]["delta_primary_minus_identity"] > 0.0,
        "medium_mean_no_regression": metrics["medium"]["delta_primary_minus_identity"] >= 0.0,
        "large_mean_no_regression": metrics["large"]["delta_primary_minus_identity"] >= 0.0,
        "complete_misses_no_increase": metrics["overall"]["primary_complete_misses"]
        <= metrics["overall"]["identity_complete_misses"],
    }
    operational_goal_checks = {
        subgroup: {
    ...
    final_safety_checks = {
        ...
        "identity_exactly_accepted_geometry_v3_baseline": identity_exactly_baseline,
        "overall_paired_ci95_lower_above_zero": metrics["overall"]["ci95"][0] > 0.0,
        "no_subgroup_mean_regression": all(
            metrics[subgroup]["delta_primary_minus_identity"] >= 0.0
            for subgroup in ("small", "medium", "large")
        ),
        "no_complete_miss_increase": mechanism_checks["complete_misses_no_increase"],
        "evaluation_boundaries_and_hashes_pass": True,
    }
    mechanism_pass = all(mechanism_checks.values())
    operational_pass = (
        mechanism_pass
        and all(check["pass"] for check in operational_goal_checks.values())
        and all(final_safety_checks.values())
    )
    status = "OPERATIONAL_PASS" if operational_pass else "MECHANISM_PASS" if mechanism_pass else "FAIL"

    output_dir.mkdir(parents=True, exist_ok=False)
    paired_rows_path = output_dir / "paired_per_image.csv"
    with paired_rows_path.open("w", encoding="utf-8", newline="") as handle:
    ...
    comparison = {
        "comparison": "global_local_instance minus geometry_v3_identity",
        ...
        "identity_evaluation_audit_sha256": expected_identity_evaluation_audit_sha256,
        "primary_evaluation_audit_sha256": expected_primary_evaluation_audit_sha256,
        "paired_per_image_sha256": sha256_file(paired_rows_path),
        "metrics": metrics,
    ...
    gate = {
        "decision_id": "mask_bag_global_local_instance_s7_decision_v1",
        "status": status,
        "mechanism_checks": mechanism_checks,
        "operational_goal_checks": operational_goal_checks,
    ...
    output_hashes = {
        "paired_per_image.csv": sha256_file(paired_rows_path),
    ...
    audit = {
        ...
        "identity_evaluation_audit_sha256": expected_identity_evaluation_audit_sha256,
        "primary_evaluation_audit_sha256": expected_primary_evaluation_audit_sha256,
        "identity_evaluation_output_hashes": identity_audit["output_hashes"],
...
def main() -> None:
    ...
    parser.add_argument("--identity-evaluation-root", type=Path, required=True)
    parser.add_argument("--identity-evaluation-audit", type=Path, required=True)
    parser.add_argument("--expected-identity-evaluation-audit-sha256", required=True)
    ...
    result = decide(
        ...
        args.identity_evaluation_root.resolve(),
        args.identity_evaluation_audit.resolve(),
        ...
        expected_identity_evaluation_audit_sha256=args.expected_identity_evaluation_audit_sha256,
        expected_primary_evaluation_audit_sha256=args.expected_primary_evaluation_audit_sha256,
    )
