...
"""Apply the exact T1 gate after the generic frozen-arm evaluator."""
...
OUTPUT_AUDIT_ID = "independent_mask_bag_count_controlled_self_paced_t1_output_v1"
...
def decide(
    ...
    *,
    ...
    if sha256_file(output_audit_path) != expected_output_audit_sha256:
        raise ValueError("T1 independent output-audit SHA-256 mismatch")
    if sha256_file(evaluation_audit_path) != expected_evaluation_audit_sha256:
        raise ValueError("T1 evaluation-audit SHA-256 mismatch")
    ...
    if (
        ...
        or output_audit.get("confirmation_residual_trained_after_producer_gate") is not True
        or output_audit.get("validation_gt_read") is not False
    ):
        raise ValueError("T1 independent output audit did not pass its GT-blind gate")
    ...
    if (
        ...
        or evaluation_audit.get("selector_cache_freeze_sha256") != CACHE_FREEZE_SHA256
        ...
        raise ValueError("T1 evaluation boundary/cohort contract mismatch")
    ...
    if (
        ...
        or paired.get("seed_family") != 20261014
        ...
        or summary.get("validation_gt_read_only_after_all_predictions_frozen_and_verified")
        is not True
    ):
        raise ValueError("T1 evaluator source/protocol/bootstrap contract mismatch")
    ...
    if (
        ...
        raise ValueError("T1 gate schema mismatch")
    mechanism_pass = all(
    ...
    if gate.get("status") != expected_status or gate.get("consumer_authorized") is not full_adoption_pass:
        raise ValueError("T1 generic gate status/consumer authorization is inconsistent")
    return {
        "decision_id": "mask_bag_count_controlled_t1_decision_v1",
...
def main() -> None:
    ...
    if args.decision_output.exists():
        raise FileExistsError("T1 decision output already exists")
    result = decide(
