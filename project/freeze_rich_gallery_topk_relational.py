...
"""Freeze bounded top-k relational choices before validation annotations."""
...
def main() -> None:
    ...
    if len(rows) != 371:
        raise RuntimeError("top-k relational freeze requires canonical validation")
    ...
    if candidate_audit.get("cohort") != "all":
        raise ValueError("top-k relational diagnostic requires complete candidates")
    ...
    for row in rows:
        ...
        scores, choices, support, top = relational_scores_and_choices(
            masks,
            sources,
            g1,
            upstream,
        ...
        support_hash = hashlib.sha256(support.astype("<f4").tobytes()).hexdigest()
        support_hashes.append(support_hash)
        top_set = set(int(value) for value in top)
            ...
            output_rows.append(
                {
                    ...
                    "selected_relational_support": float(support[local]),
                    "selected_rule_score": float(scores[variant][local]),
                    "selected_in_top10": int(local in top_set),
                    "support_vector_sha256": support_hash,
                }
            )
    ...
    freeze = {
        "stage": "rich_gallery_topk_relational_freeze_v1",
        ...
        "support_set_sha256": hashlib.sha256(
            "\n".join(sorted(support_hashes)).encode()
        ).hexdigest(),
        "top_k": TOP_K,
