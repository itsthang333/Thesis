...
def adversarial_climbing_layercam(
    ...
    *,
    ...
    This bounded adaptation uses only the requested image-level class logit.
    ``iterations`` is the number of input updates, so the aggregate includes
    the unmodified image and ``iterations`` perturbed images. Per-step maps
    are normalised by ``LayerCAM`` before aggregation, and every perturbation
    is clamped to the original sample's observed normalised range.
    """
    if iterations < 0:
