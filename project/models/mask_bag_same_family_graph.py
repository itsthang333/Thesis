...
from models.mask_bag_relational_selector import smooth_candidate_logits
...
def build_cached_same_family_graph(
    ...
    *,
    ...
    """Build a symmetric same-family graph from frozen pairwise cache matrices."""
    ...
    if not valid.any(dim=1).all() or (family_ids[valid] < 0).any():
        raise ValueError("every bag needs valid candidates with nonnegative family IDs")
    if (
...
def _padded_graph(
    ...
    *,
    ...
    adjacency = build_cached_same_family_graph(
        iou,
        containment,
        valid,
        family_ids,
