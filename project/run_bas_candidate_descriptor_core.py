...
MAXIMUM_CORRELATION_FIELD = "maximum_mean_bas_upstream_rank_correlation"
...
def main() -> None:
    ...
    if REQUIRE_DIAGNOSTIC_PASS_TO_FREEZE and not gate["operational_gate_pass"]:
    ...
    gate.update(
        {
            **complementarity,
            MAXIMUM_CORRELATION_FIELD: MAXIMUM_CORRELATION,
            "minimum_changed_selection_fraction": MINIMUM_CHANGE_FRACTION,
    ...
    gate["complementarity_gate_pass"] = bool(
        float(complementarity[CORRELATION_KEY]) <= MAXIMUM_CORRELATION
        and float(complementarity[CHANGE_FRACTION_KEY])
