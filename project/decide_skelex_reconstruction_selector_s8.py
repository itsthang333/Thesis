"""Apply the predeclared S8 gates to two hash-frozen evaluation tables."""
...
def _read_per_image(path: Path, expected_sha256: str) -> dict[str, dict[str, str]]:
    ...
    if len(rows) != 184 or not rows:
        raise ValueError("Each S8 evaluation must contain exactly 184 tumor rows")
    missing = sorted(REQUIRED_PER_IMAGE_FIELDS - set(rows[0]))
    if missing:
        raise ValueError(f"S8 per-image evaluation lacks fields: {missing}")
    indexed: dict[str, dict[str, str]] = {}
    for row in rows:
        image_id = row["image_id"]
        if not image_id or image_id in indexed:
            raise ValueError("S8 evaluation has duplicate/empty image IDs")
        if row["size_group"] not in EXPECTED_COUNTS:
            raise ValueError("S8 evaluation has an invalid subgroup")
        if row["complete_miss"] not in {"0", "1"} or row["baseline_complete_miss"] not in {"0", "1"}:
            raise ValueError("S8 evaluation has an invalid miss flag")
        for field in (
            "gt_area_ratio",
            "dice",
            "oracle_best_single_dice",
    ...
    if counts != EXPECTED_COUNTS:
        raise ValueError(f"Frozen S8 subgroup counts differ: {counts}")
    return indexed
...
def _paired_group_bootstrap(
    ...
    *,
    ...
    if deltas.shape != (len(groups),):
        raise ValueError("S8 bootstrap values/groups do not align")
...
def _verify_evaluation(
    ...
    *,
    ...
    if sha256_file(audit_path) != expected_audit_sha256:
        raise ValueError("S8 evaluation-audit SHA-256 mismatch")
    ...
    if (
        ...
        raise ValueError("S8 evaluator source/protocol/bootstrap mismatch")
...
def decide(
    ...
    *,
    ...
    if sha256_file(readiness_path) != expected_readiness_sha256:
        raise ValueError("S8 postfreeze readiness SHA-256 mismatch")
    readiness = _json(readiness_path)
    _require_safety(readiness, name="postfreeze readiness")
    if (
        ...
        or readiness.get("evaluation_addendum_sha256") != expected_evaluation_addendum_sha256
        or readiness.get("terminal_pre_gt_audit_sha256") != expected_pre_gt_audit_sha256
        or readiness.get("validation_gt_read") is not False
    ):
        raise ValueError("S8 postfreeze readiness boundary mismatch")
    if sha256_file(pre_gt_audit_path) != expected_pre_gt_audit_sha256:
        raise ValueError("S8 pre-GT audit SHA-256 mismatch")
    ...
    if (
        ...
        or pre_gt.get("pair_freeze_sha256") != readiness.get("prediction_pair_freeze_sha256")
        or pre_gt.get("validation_gt_read") is not False
    ):
        raise ValueError("S8 pre-GT audit did not authorize evaluation")
    arms = readiness.get("arms", {})
    control_contract = arms.get(CONTROL_ARM, {}) if isinstance(arms, dict) else {}
    primary_contract = arms.get(PRIMARY_ARM, {}) if isinstance(arms, dict) else {}
    control, control_audit = _verify_evaluation(
        ...
        expected_arm_freeze_sha256=str(control_contract.get("prediction_freeze_sha256", "")),
        expected_score_manifest_sha256=str(control_contract.get("candidate_score_manifest_sha256", "")),
    )
    primary, primary_audit = _verify_evaluation(
        ...
        expected_arm_freeze_sha256=str(primary_contract.get("prediction_freeze_sha256", "")),
        expected_score_manifest_sha256=str(primary_contract.get("candidate_score_manifest_sha256", "")),
    )
    if set(control) != set(primary):
        raise ValueError("S8 evaluated arm image identities differ")
    ...
    for image_id in sorted(control):
        ...
        for field in (
            ...
            if old[field] != new[field]:
                raise ValueError(f"S8 frozen paired field differs for {image_id}: {field}")
    ...
    for subgroup_index, subgroup in enumerate(SUBGROUPS):
        rows = [row for row in paired_rows if subgroup == "overall" or row["size_group"] == subgroup]
        ...
        metrics[subgroup] = {
            **_paired_group_bootstrap(
            ...
            "primary_dice": float(np.mean([float(row["primary_dice"]) for row in rows])),
            "control_dice": float(np.mean([float(row["control_dice"]) for row in rows])),
            "primary_complete_misses": int(sum(int(row["primary_complete_miss"]) for row in rows)),
            "control_complete_misses": int(sum(int(row["control_complete_miss"]) for row in rows)),
        }
    mechanism_checks = {
        "overall_mean_strictly_improves": metrics["overall"]["delta_primary_minus_control"] > 0.0,
        "small_mean_strictly_improves": metrics["small"]["delta_primary_minus_control"] > 0.0,
        "medium_mean_no_regression": metrics["medium"]["delta_primary_minus_control"] >= 0.0,
        "large_mean_no_regression": metrics["large"]["delta_primary_minus_control"] >= 0.0,
        "complete_misses_no_increase": metrics["overall"]["primary_complete_misses"] <= metrics["overall"]["control_complete_misses"],
    }
    goal_checks = {
        subgroup: {
    ...
    operational_pass = mechanism_pass and all(check["pass"] for check in goal_checks.values()) and all(safety_checks.values())
    ...
    gate = {
        "decision_id": "skelex_reconstruction_selector_s8_decision_v1",
        "status": status,
    ...
    audit = {
        ...
        "prediction_pair_freeze_sha256": readiness["prediction_pair_freeze_sha256"],
...
def main() -> None:
    ...
    result = decide(
        ...
        expected_control_evaluation_audit_sha256=args.expected_control_evaluation_audit_sha256,
