...
"""Independent GT-blind physical auditor for frozen R4 output."""
...
REQUIRED_RUNTIME_SOURCES = {
    "project/run_mask_bag_orbit_relation_arm.py",
...
def _verify_launch_binding(
    ...
    if (
        ...
        raise ValueError("R4 launch binding contract mismatch")
    for field in ("kernel", "checkout_commit"):
        if not str(binding.get(field, "")).strip():
            raise ValueError(f"R4 launch binding omits {field}")
    ...
    if not isinstance(source_hashes, dict) or not REQUIRED_RUNTIME_SOURCES <= set(source_hashes):
        raise ValueError("R4 launch binding omits required runtime sources")
    ...
    for relative, expected in source_hashes.items():
        ...
        if protocol_hashes.get(relative) != expected:
            raise ValueError(f"R4 runtime source is not frozen by protocol: {relative}")
    return {str(key): str(value) for key, value in source_hashes.items()}


def _verify_history(path: Path) -> None:
    history = json.loads(path.read_text(encoding="utf-8"))
    required = {"epoch", "total", "image", "instance"}
    if not isinstance(history, list) or len(history) != 16:
        raise ValueError("R4 history must contain exactly 16 fixed epochs")
    for index, row in enumerate(history, start=1):
        if not isinstance(row, dict) or set(row) != required or int(row["epoch"]) != index:
            raise ValueError("R4 history schema or epoch order mismatch")
        if not all(math.isfinite(float(value)) for value in row.values()):
            raise ValueError("R4 history contains a non-finite value")
...
def _verify_pretraining_audit(
    ...
    payload = _json(path)
    _require_safety(payload, name="R4 pretraining identity audit")
    if (
        payload.get("source_commit") != SOURCE_COMMIT
        or payload.get("protocol_sha256") != PROTOCOL_SHA256
        or set(payload) != {
            "train", "validation", "source_commit", "protocol_sha256",
            "validation_gt_read", "consumer_trained", "test_evaluated"
        }
    ):
        raise ValueError("R4 pretraining identity provenance mismatch")
    required_true = {
        "zero_residual_exact",
        "combined_equals_averaged_frozen_base_exact",
        "view_swap_candidate_logits_exact",
        "view_swap_critical_indices_exact",
    }
    for split, expected in (("train", expected_train), ("validation", expected_validation)):
        ...
        if (
            ...
            raise ValueError(f"R4 pretraining identity mismatch: {split}")
    return payload


def _verify_gt_blind_diagnostics(
    root: Path, freeze: Mapping[str, Any], *, expected_validation: int
) -> dict[str, Any]:
    path = root / "gt_blind_diagnostics.csv"
    if sha256_file(path) != freeze.get("gt_blind_diagnostics_sha256"):
        raise ValueError("R4 GT-blind diagnostics differ from prediction freeze")
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    required = {"image_id", "candidate_count", "bag_probability", "view_swap_exact"}
    if (
        ...
        raise ValueError("R4 GT-blind diagnostic cohort/schema mismatch")
    counts: list[int] = []
    ...
    for row in rows:
        ...
        swap = int(row["view_swap_exact"])
        if count <= 0 or not math.isfinite(probability) or not 0 <= probability <= 1:
            raise ValueError("R4 GT-blind diagnostic value mismatch")
        if swap not in (0, 1):
            raise ValueError("R4 swap diagnostic must be binary")
        counts.append(count)
        probabilities.append(probability)
        swap_values.append(swap)
    count_spearman = abs(_spearman(counts, probabilities))
    swap_exact = sum(swap_values)
    expected_count_pass = count_spearman <= COUNT_SPEARMAN_CEILING
    expected_swap_pass = swap_exact == expected_validation
    ...
    if (
        ...
        or gate.get("view_swap_exact_records") != swap_exact
        or gate.get("view_swap_expected_records") != expected_validation
        or gate.get("view_swap_gate_pass") is not expected_swap_pass
        or gate.get("gt_blind_gate_pass") is not (expected_count_pass and expected_swap_pass)
    ):
        raise ValueError("R4 GT-blind gate does not reproduce")
    return {
        "sha256": sha256_file(path),
        "records": expected_validation,
        "absolute_candidate_count_probability_spearman": count_spearman,
        "view_swap_exact_records": swap_exact,
        "gt_blind_gate_pass": expected_count_pass and expected_swap_pass,
...
def audit_r4_output(
    ...
    *,
    ...
    if sha256_file(protocol_path) != PROTOCOL_SHA256:
        raise ValueError("R4 protocol SHA-256 mismatch")
    ...
    paths = {
        ...
        "checkpoint": root / "orbit_relation_residual.pt",
        "history": root / "training_history.json",
        "initial": root / "pretraining_identity_audit.json",
    }
    for path in paths.values():
        if not path.is_file():
            raise FileNotFoundError(f"R4 output is missing: {path}")
    ...
    if (
        ...
        raise ValueError("R4 prediction-freeze provenance mismatch")
    if (
        run.get("run_id") != "btxrd_mask_bag_orbit_relation_r4_v1"
        ...
        raise ValueError("R4 run-manifest contract mismatch")
    ...
    if (
        ...
        raise ValueError("R4 T4x2 runtime contract mismatch")
    if (
        ...
        or wrapper.get("physical_gt_blind_diagnostic_rows_verified") != expected_validation
    ):
        raise ValueError("R4 wrapper-output audit mismatch")
    t4 = wrapper.get("t4x2", {})
    if (
        ...
        raise ValueError("R4 wrapper T4x2 evidence mismatch")
    ...
    if (
        ...
        raise ValueError("R4 wrapper cache evidence mismatch")
    if sha256_file(paths["checkpoint"]) != freeze.get("checkpoint_sha256"):
        raise ValueError("R4 checkpoint differs from prediction freeze")
    if sha256_file(paths["history"]) != freeze.get("training_history_sha256"):
        raise ValueError("R4 training history differs from prediction freeze")
    if sha256_file(paths["initial"]) != freeze.get("pretraining_identity_audit_sha256"):
        raise ValueError("R4 pretraining audit differs from prediction freeze")
    if paths["checkpoint"].stat().st_size <= 0:
        raise ValueError("R4 checkpoint is empty")
    ...
    validation = _verify_validation_evidence(
        root, freeze, expected_validation=expected_validation, expected_map_shape=expected_map_shape
    )
    diagnostics = _verify_gt_blind_diagnostics(
        root, freeze, expected_validation=expected_validation
    )
    if abs(float(validation["validation_absolute_count_probability_spearman"]) - float(diagnostics["absolute_candidate_count_probability_spearman"])) > 1.0e-12:
        raise ValueError("R4 prediction and diagnostic count associations differ")
    physical_bytes = sum(path.stat().st_size for path in (*paths.values(), launch_binding_path))
    return {
        "audit_id": "independent_mask_bag_orbit_relation_r4_output_v1",
        "status": (
        ...
        "physical_output_bytes_verified": physical_bytes + int(validation["physical_validation_evidence_bytes"]) + int(diagnostics["physical_bytes"]),
...
def main() -> None:
    ...
    result = audit_r4_output(
        args.output_root.resolve(), args.protocol.resolve(), args.launch_binding.resolve()
    )
