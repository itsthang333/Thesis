"""Independent post-freeze audit of the rich-gallery BAS B2 evaluation."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--expected-evaluation-freeze-sha256", required=True)
    parser.add_argument("--expected-evaluator-source-sha256", required=True)
...
def _load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ValueError(f"B2 audit expected JSON object: {path}")
    return value
...
def _safe_path(root: Path, relative: str) -> Path:
    ...
    if resolved == resolved_root or resolved_root not in resolved.parents:
        raise ValueError("B2 prediction path escapes arm root")
    return resolved


def _unpack_prediction(path: Path) -> np.ndarray:
    with np.load(path, allow_pickle=False) as payload:
        ...
        packed = np.asarray(payload["packed_mask"], dtype=np.uint8)
...
def _verify_independence(auditor_path: Path) -> None:
    tree = ast.parse(auditor_path.read_text(encoding="utf-8"))
    forbidden = {
        "evaluate_rich_gallery_bas_semantic_b2",
        "run_rich_gallery_bas_semantic_b2",
    }
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            ...
            continue
        if any(any(name in module for name in forbidden) for module in modules):
            raise ValueError("B2 evaluation auditor imports producer/evaluator")


def _assert_nested_close(actual: Any, expected: Any, label: str) -> None:
...
def _average_rank(values: np.ndarray) -> np.ndarray:
    ...
    if array.ndim != 1 or not len(array) or not np.isfinite(array).all():
        raise ValueError("B2 audit rank input must be finite and nonempty")
...
def _rank_correlation(scores: np.ndarray, quality: np.ndarray) -> float:
    score_rank = _average_rank(scores)
    quality_rank = _average_rank(quality)
    ...
    if not np.isfinite(value):
        raise ValueError("B2 audit score/quality correlation is non-finite")
    return value


def _summarize(
    records: list[dict[str, object]], arm: str
...
def paired_group_bootstrap(
    ...
    *,
    ...
    if not (len(arm) == len(control) == len(groups)) or not arm:
        raise ValueError("B2 audit bootstrap arrays do not align")
    delta = np.asarray(arm, dtype=np.float64) - np.asarray(control, dtype=np.float64)
    by_group: dict[str, list[float]] = {}
    for index, group in enumerate(groups):
        by_group.setdefault(group, []).append(float(delta[index]))
...
def _mechanism(records: list[dict[str, object]]) -> dict[str, object]:
    by_arm = {
        arm: {
            str(row["image_id"]): row for row in records if row["arm"] == arm
        }
        for arm in (CONTROL_ARM, SEMANTIC_ARM)
    }
    changed = improved = worsened = tied = recovered = lost = 0
    ...
    for image_id in sorted(by_arm[CONTROL_ARM]):
        ...
        changed_choice = int(control["selected_candidate_index"]) != int(
            semantic["selected_candidate_index"]
        )
        changed += int(changed_choice)
        if changed_choice:
            transitions[
        ...
        for arm, row in ((CONTROL_ARM, control), (SEMANTIC_ARM, semantic)):
            ratio = float(row["selected_area_ratio"]) / float(row["gt_area_ratio"])
            areas["overall"][arm].append(ratio)
            areas[subgroup][arm].append(ratio)
    return {
        ...
        "complete_misses_recovered": recovered,
        "complete_hits_lost": lost,
        ...
        "median_selected_to_gt_area_ratio": {
            ...
            for subgroup, values in areas.items()
        },
    }


def _verify_no_gt_boundary(
    args: argparse.Namespace,
    validation_rows: list[dict[str, str]],
) -> tuple[dict[str, dict[str, dict[str, str]]], dict[str, Any]]:
    if sha256_file(args.independent_no_gt_audit) != (
        args.expected_independent_no_gt_audit_sha256
    ):
        raise ValueError("B2 independent no-GT audit hash mismatch")
    no_gt = _load_json(args.independent_no_gt_audit)
    if (
    ...
    if sha256_file(pair_path) != args.expected_pair_freeze_sha256:
        raise ValueError("B2 pair freeze hash mismatch")
    ...
    if (
        ...
        raise ValueError("B2 pair freeze provenance/safety mismatch")
    expected_ids = {row["image_id"] for row in validation_rows}
    ...
    for arm in (CONTROL_ARM, SEMANTIC_ARM):
        ...
        if (
            ...
            raise ValueError(f"B2 {arm} prediction freeze mismatch")
        rows = _load_csv(manifest_path)
        current = {row["image_id"]: row for row in rows}
        if len(rows) != 371 or len(current) != 371 or set(current) != expected_ids:
            raise ValueError(f"B2 {arm} prediction cohort mismatch")
        for image_id, row in current.items():
            path = _safe_path(arm_root, row["prediction_path"])
            if sha256_file(path) != row["prediction_sha256"]:
                raise ValueError(f"B2 prediction changed: {arm}/{image_id}")
            if _unpack_prediction(path).shape != (320, 320):
                raise ValueError(f"B2 prediction shape changed: {arm}/{image_id}")
            physical += 1
        indexed[arm] = current
    if physical != 742:
        raise RuntimeError("B2 physical prediction audit count mismatch")
    return indexed, no_gt


def audit(args: argparse.Namespace) -> dict[str, Any]:
    _verify_independence(Path(__file__))
    evaluator_path = Path(__file__).with_name(
        "evaluate_rich_gallery_bas_semantic_b2.py"
    )
    if (
        sha256_file(evaluator_path) != args.expected_evaluator_source_sha256
        or args.bootstrap_replicates != 10000
        or args.bootstrap_seed != 42
    ):
        raise ValueError("B2 evaluation source/bootstrap contract mismatch")
    ...
    if len(validation_rows) != 371:
        raise ValueError("B2 validation cohort mismatch")
    predictions, no_gt = _verify_no_gt_boundary(args, validation_rows)
    pair = _load_json(args.prediction_root / "prediction_pair_freeze.json")
    ...
    if candidate_audit.get("cohort") != "all":
        raise ValueError("B2 audit requires the complete rich gallery")
    ...
    if sha256_file(score_manifest_path) != pair["candidate_score_manifest_sha256"]:
        raise ValueError("B2 audit candidate-score manifest changed")
    score_rows = {row["image_id"]: row for row in _load_csv(score_manifest_path)}
    if len(score_rows) != 371:
        raise ValueError("B2 audit candidate-score cohort mismatch")

    # This is the same one-way boundary as the evaluator: raw validation masks
    # are imported only after the pair and all 742 physical files are audited.
    ...
    for index in range(len(dataset)):
        ...
        if int(predictions[CONTROL_ARM][image_id]["tumor"]) == 0:
            continue
        ...
        if sha256_file(candidate_path) != candidate_row["diagnostic_sha256"]:
            raise ValueError(f"B2 audit candidate payload changed: {image_id}")
        ...
        if len(candidate_masks) != len(source_names):
            raise ValueError(f"B2 audit candidate/source mismatch: {image_id}")
        candidate_quality = np.asarray(
            [_dice(mask, target) for mask in candidate_masks], dtype=np.float64
        )
        ...
        score_path = _safe_path(args.prediction_root, score_row["score_path"])
        if sha256_file(score_path) != score_row["score_sha256"]:
            raise ValueError(f"B2 audit candidate scores changed: {image_id}")
        ...
        if (
            ...
            raise ValueError(f"B2 audit candidate-score alignment mismatch: {image_id}")
        ...
        for arm in (CONTROL_ARM, SEMANTIC_ARM):
            ...
            prediction = _unpack_prediction(
                _safe_path(args.prediction_root / arm, row["prediction_path"])
            )
            ...
            if (
                ...
                raise ValueError(f"B2 audit evaluated choice mismatch: {arm}/{image_id}")
            selected_source = canonical_source(source_names[selected_index])
            selected_source_oracle = float(
                eligible_quality[eligible_sources == selected_source].max()
            )
            ...
            selected_dice = _dice(prediction, target)
            ...
            if min(regret_components) < -1.0e-12 or not np.isclose(
                ...
                raise ValueError(
                    f"B2 audit selector-regret decomposition failed: {arm}/{image_id}"
                )
            topk = {
            ...
            records.append(
                {
                    ...
                    "score_quality_rank_correlation": _rank_correlation(
    ...
    if len(records) != 368:
        raise RuntimeError("B2 independent evaluation pair count mismatch")
    subgroup_counts = Counter(
        row["size_group"] for row in records if row["arm"] == CONTROL_ARM
    )
    if subgroup_counts != Counter({"small": 94, "medium": 72, "large": 18}):
        raise RuntimeError("B2 independent evaluation subgroup mismatch")

    metrics = {arm: _summarize(records, arm) for arm in (CONTROL_ARM, SEMANTIC_ARM)}
    for subgroup, value in TRUSTED_CONTROL.items():
        if abs(float(metrics[CONTROL_ARM][subgroup]["dice"]) - value) > 1.0e-8:
            raise ValueError("trusted collaborator control integrity anchor drifted")
    by_arm = {
        arm: {
            str(row["image_id"]): row for row in records if row["arm"] == arm
        }
    ...
    bootstrap = paired_group_bootstrap(
        [float(by_arm[SEMANTIC_ARM][key]["dice"]) for key in ordered_ids],
        [float(by_arm[CONTROL_ARM][key]["dice"]) for key in ordered_ids],
        [str(by_arm[CONTROL_ARM][key]["group_id"]) for key in ordered_ids],
    ...
    mechanism_pass = bool(
        ...
        and all(value >= 0.0 for value in subgroup_deltas.values())
    ...
    expected_summary = {
        ...
        "independent_no_gt_audit_sha256": (
            args.expected_independent_no_gt_audit_sha256
        ),
        "cohort": {
            "validation": 371,
        ...
        "paired_mechanism_diagnostics": _mechanism(records),
        ...
        "evaluator_source_sha256": args.expected_evaluator_source_sha256,
    ...
    freeze_path = args.evaluation_root / "evaluation_freeze.json"
    if sha256_file(freeze_path) != args.expected_evaluation_freeze_sha256:
        raise ValueError("B2 evaluation-freeze hash mismatch")
    freeze = _load_json(freeze_path)
    per_image_path = args.evaluation_root / "per_image.csv"
    ...
    if (
        ...
        or freeze.get("pair_freeze_sha256") != args.expected_pair_freeze_sha256
        or freeze.get("independent_no_gt_audit_sha256")
        != args.expected_independent_no_gt_audit_sha256
        or freeze.get("per_image_sha256") != sha256_file(per_image_path)
        or freeze.get("summary_sha256") != sha256_file(summary_path)
        or freeze.get("evaluator_source_sha256")
        != args.expected_evaluator_source_sha256
        or freeze.get("prediction_frozen_before_validation_gt") is not True
        or freeze.get("test_images_read") != 0
        or freeze.get("test_evaluated") is not False
    ):
        raise ValueError("B2 evaluation freeze contract mismatch")

    cloud_rows = _load_csv(per_image_path)
    if len(cloud_rows) != len(records):
        raise ValueError("B2 cloud/local per-image count differs")
    cloud_index = {(row["arm"], row["image_id"]): row for row in cloud_rows}
    if len(cloud_index) != len(records):
        raise ValueError("B2 cloud per-image rows are duplicated")
    for local in records:
        cloud = cloud_index[(str(local["arm"]), str(local["image_id"]))]
        text_fields = {
            "arm",
            "image_id",
            "group_id",
            "size_group",
            "selected_source",
            "gallery_oracle_source",
            "eligible_oracle_source",
        }
        if set(cloud) != set(local):
            raise ValueError("B2 cloud/local per-image schema differs")
        for key, value in local.items():
            if key in text_fields:
                matches = str(cloud[key]) == str(value)
            else:
                matches = bool(
                    np.isclose(float(cloud[key]), float(value), rtol=0, atol=1e-12)
                )
            if not matches:
                raise ValueError(f"B2 cloud/local per-image field differs: {key}")
    _assert_nested_close(_load_json(summary_path), expected_summary, "B2 summary")
    return {
        "stage": "rich_gallery_bas_semantic_b2_independent_evaluation_audit_v1",
        "audit_pass": True,
        "experiment_id": EXPERIMENT_ID,
        "source_commit": args.expected_source_commit,
        "protocol_sha256": args.expected_protocol_sha256,
        ...
        "evaluation_freeze_sha256": args.expected_evaluation_freeze_sha256,
        "evaluator_source_sha256": args.expected_evaluator_source_sha256,
        "per_image_rows_reproduced": 368,
        "summary_reproduced": True,
        "trusted_control_used_as_integrity_anchor_not_retrained": True,
        "validation_gt_read_only_after_pair_and_no_gt_audit": True,
        "consumer_authorized": bool(mechanism_pass and goal_pass),
        "test_images_read": 0,
        "test_evaluated": False,
    }
...
def main() -> None:
    ...
    if args.output.exists():
        raise FileExistsError("B2 independent evaluation audit output exists")
    result = audit(args)
