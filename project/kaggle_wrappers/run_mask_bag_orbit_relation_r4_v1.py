...
"""Fail-closed Kaggle bootstrap for the predeclared R4 orbit arm."""
...
KERNEL = "itsthang333/btxrd-rad-dino-mask-bag-orbit-relation-r4-v1"
KERNEL_VERSION = 0
LAUNCH_BINDING_READY = False
CHECKOUT_COMMIT = "UNBOUND"
REPOSITORY = "https://github.com/itsthang333/Thesis.git"
SOURCE_COMMIT = "84f3dca6bb58fa3dc2a0a48b5f58c05e33edff6e"
PROTOCOL_RELATIVE = Path(
    "artifacts/research_protocols/rad_dino_mask_bag_orbit_relation_r4_v1.json"
)
PROTOCOL_SHA256 = "00d1ba1b7611c8320e87948d88dac06af87c0b29d1e62f875e79105fc41c0aae"
...
SOURCE = WORK / "r4_source"
RUNTIME = WORK / "r4_runtime"
OUTPUT = WORK / "btxrd_mask_bag_orbit_relation_r4_v1"
...
def unique(name: str, expected_hash: str) -> Path:
    matches = [path for path in INPUT.rglob(name) if path.is_file() and hash_file(path) == expected_hash]
    if len(matches) != 1:
...
def clone_and_verify() -> dict[str, str]:
    if not LAUNCH_BINDING_READY or KERNEL_VERSION < 1:
        raise RuntimeError("R4 launch binding is not frozen")
    if len(CHECKOUT_COMMIT) != 40 or any(c not in "0123456789abcdef" for c in CHECKOUT_COMMIT):
        raise RuntimeError("Invalid bound R4 checkout")
    ...
    if hash_file(protocol_path) != PROTOCOL_SHA256:
        raise RuntimeError("R4 protocol hash mismatch")
    protocol = json.loads(protocol_path.read_text(encoding="utf-8"))
    hashes = protocol.get("canonical_lf_source_hashes", {})
    for relative, expected in hashes.items():
        path = SOURCE / relative
        if not path.is_file() or canonical_hash(path) != expected:
            raise RuntimeError(f"R4 source hash mismatch: {relative}")
    return hashes


def install_runtime() -> None:
    run([sys.executable, "-m", "pip", "install", "--disable-pip-version-check", "--no-input", "--no-cache-dir", "--force-reinstall", "--no-deps", "torch==2.5.1+cu121", "torchvision==0.20.1+cu121", "--index-url", "https://download.pytorch.org/whl/cu121"], cwd=SOURCE)
    run([sys.executable, "-m", "pip", "install", "--disable-pip-version-check", "--no-input", "--no-cache-dir", "transformers==4.50.2"], cwd=SOURCE)


def verify_t4x2() -> dict[str, object]:
    import torch
    if not torch.cuda.is_available() or torch.cuda.device_count() != 2:
        raise RuntimeError("R4 requires exactly two CUDA devices")
    names = [torch.cuda.get_device_name(index) for index in range(2)]
    if not all("T4" in name for name in names):
        raise RuntimeError(f"R4 requires T4 x2, got {names}")
    checksums = []
    for index in range(2):
        layer = torch.nn.Conv2d(3, 5, 3, padding=1).to(f"cuda:{index}").eval()
        values = torch.arange(3072, dtype=torch.float32, device=f"cuda:{index}").reshape(1, 3, 32, 32) / 1024
...
def prepare_split() -> Path:
    ...
    if hash_file(source) != GIT_SPLIT_SHA256 or b"\r" in source.read_bytes():
        raise RuntimeError("R4 canonical split mismatch")
    target = RUNTIME / "frozen_split_manifest.csv"
    target.write_bytes(source.read_bytes().replace(b"\n", b"\r\n"))
    if hash_file(target) != SPLIT_SHA256:
        raise RuntimeError("R4 frozen split reconstruction mismatch")
    return target
...
def prepare_baseline() -> tuple[Path, dict[str, object]]:
    ...
    if audit.get("validation_gt_included") is not False or audit.get("consumer_trained") is not False or audit.get("test_evaluated") is not False:
        raise RuntimeError("R4 baseline transport safety mismatch")
    ...
    for freeze in extracted.rglob("prediction_freeze.json"):
        root = freeze.parent
        if hash_file(freeze) == BASELINE["freeze"] and hash_file(root / "rad_dino_mask_bag_mil.pt") == BASELINE["checkpoint"] and hash_file(root / "predictions/prediction_manifest.csv") == BASELINE["manifest"]:
            roots.append(root.resolve())
    if len(set(roots)) != 1:
        raise RuntimeError(f"Expected one R4 baseline root, found {roots}")
    return roots[0], {"baseline_archive_sha256": hash_file(archive), "transport_audit_sha256": hash_file(audit_path)}
...
def find_cache() -> tuple[Path, dict[str, object]]:
    ...
    if hash_file(audit_path) != CACHE_WRAPPER_AUDIT_SHA256:
        raise RuntimeError("R4 cache audit hash mismatch")
    freeze = json.loads(freeze_path.read_text(encoding="utf-8"))
    audit = json.loads(audit_path.read_text(encoding="utf-8"))
    if freeze.get("cohort") != {"train": 2981, "validation": 371} or audit.get("physical_cache_records_verified") != 3352:
        raise RuntimeError("R4 cache contract mismatch")
    for payload in (freeze, audit):
        if payload.get("validation_gt_read") is not False or payload.get("consumer_trained") is not False or payload.get("test_evaluated") is not False:
            raise RuntimeError("R4 cache safety mismatch")
    return root, {"selector_cache_freeze_sha256": hash_file(freeze_path), "selector_cache_wrapper_audit_sha256": hash_file(audit_path), "physical_cache_records_verified": 3352}


def audit_output(source_hashes: dict[str, str], cache: dict[str, object], baseline: dict[str, object], t4: dict[str, object]) -> None:
    import numpy as np
    ...
    required = (freeze_path, run_path, prediction_path, score_path, diagnostic_path, OUTPUT / "orbit_relation_residual.pt", OUTPUT / "training_history.json", OUTPUT / "pretraining_identity_audit.json")
    for path in required:
        if not path.is_file():
            raise RuntimeError(f"R4 output missing: {path}")
    freeze = json.loads(freeze_path.read_text(encoding="utf-8"))
    run_manifest = json.loads(run_path.read_text(encoding="utf-8"))
    if freeze.get("protocol_sha256") != PROTOCOL_SHA256 or freeze.get("validation_predictions") != 371 or freeze.get("training_labels") != "image_level_only" or run_manifest.get("validated_cache_records") != {"train": 2981, "validation": 371}:
        raise RuntimeError("R4 freeze/run contract mismatch")
    for payload in (freeze, run_manifest):
        if payload.get("validation_gt_read") is not False or payload.get("consumer_trained") is not False or payload.get("test_evaluated") is not False:
            raise RuntimeError("R4 output safety mismatch")
    ...
    with diagnostic_path.open("r", encoding="utf-8-sig", newline="") as handle:
        diagnostics = list(csv.DictReader(handle))
    for rows in (predictions, scores, diagnostics):
        if len(rows) != 371 or len({row["image_id"] for row in rows}) != 371:
            raise RuntimeError("R4 output cohort mismatch")
    for row in predictions:
        path = OUTPUT / "predictions" / row["map_path"]
        values = np.load(path, allow_pickle=False)
        if hash_file(path) != row["map_sha256"] or values.shape != (320, 320) or values.dtype != np.float16 or not np.isfinite(values).all():
            raise RuntimeError(f"R4 prediction map mismatch: {row['image_id']}")
    for row in scores:
        ...
        if hash_file(path) != row["score_sha256"] or indices.dtype != np.int64 or logits.dtype != np.float32 or logits.shape != indices.shape or not np.isfinite(logits).all():
            raise RuntimeError(f"R4 candidate-score mismatch: {row['image_id']}")
    wrapper_audit = {
        "kernel": KERNEL, "kernel_version": KERNEL_VERSION,
        "bound_wrapper_sha256": canonical_hash(Path(__file__)),
        "checkout_commit": CHECKOUT_COMMIT, "scientific_source_commit": SOURCE_COMMIT,
        "protocol_sha256": PROTOCOL_SHA256, "source_hashes": source_hashes,
        "cache": cache, "baseline": baseline, "t4x2": t4,
        ...
        "python": platform.python_version(), "finished_utc": datetime.now(timezone.utc).isoformat()
    }
...
def main() -> None:
    ...
    run([sys.executable, "-m", "pytest", "-q", "tests/test_run_mask_bag_orbit_relation_arm.py", "tests/test_mask_bag_orbit_relation_training.py", "tests/test_mask_bag_relational_selector.py", "tests/test_mask_bag_selector_cache.py", "tests/test_mask_bag_selector_cache_io.py"], cwd=SOURCE)
    run([sys.executable, "-m", "pytest", "-q"], cwd=SOURCE)
    run([
        sys.executable, str(SOURCE / "project/run_mask_bag_orbit_relation_arm.py"),
        "--split-manifest", str(split), "--expected-split-sha256", SPLIT_SHA256,
        "--selector-cache-root", str(cache_root), "--expected-selector-cache-freeze-sha256", CACHE_FREEZE_SHA256,
        "--baseline-root", str(baseline_root), "--expected-baseline-checkpoint-sha256", BASELINE["checkpoint"],
        "--expected-baseline-source-commit", BASELINE["source_commit"], "--expected-baseline-protocol-sha256", BASELINE["protocol"],
        "--source-commit", SOURCE_COMMIT, "--protocol-sha256", PROTOCOL_SHA256,
