...
"""Fail-closed Kaggle bootstrap for the predeclared R2 affinity arm."""
...
SOURCE = WORK / "r2_source"
RUNTIME = WORK / "r2_runtime"
OUTPUT = Path("/kaggle/working/btxrd_mask_bag_affinity_residual_r2_v1")
...
def verify_t4x2() -> dict[str, object]:
    ...
    if not torch.cuda.is_available() or torch.cuda.device_count() != 2:
        raise RuntimeError("R2 requires exactly two CUDA devices")
    names = [torch.cuda.get_device_name(index) for index in range(2)]
    if not all("T4" in name for name in names):
        raise RuntimeError(f"R2 requires T4 x2, got {names}")
    checksums = []
...
def prepare_split() -> Path:
    ...
    if hash_file(source) != GIT_SPLIT_SHA256 or b"\r" in source.read_bytes():
        raise RuntimeError("R2 canonical split mismatch")
    target = RUNTIME / "frozen_split_manifest.csv"
    target.write_bytes(source.read_bytes().replace(b"\n", b"\r\n"))
    if hash_file(target) != SPLIT_SHA256:
        raise RuntimeError("R2 frozen split reconstruction mismatch")
    return target
...
def prepare_baseline() -> tuple[Path, dict[str, object]]:
    ...
    if audit.get("validation_gt_included") is not False or audit.get("consumer_trained") is not False or audit.get("test_evaluated") is not False:
        raise RuntimeError("R2 baseline transport safety mismatch")
    ...
    if len(set(roots)) != 1:
        raise RuntimeError(f"Expected one R2 baseline root, found {roots}")
...
def find_cache() -> tuple[Path, dict[str, object]]:
    ...
    if hash_file(audit_path) != CACHE_WRAPPER_AUDIT_SHA256:
        raise RuntimeError("R2 cache audit hash mismatch")
    freeze = json.loads(freeze_path.read_text(encoding="utf-8"))
    audit = json.loads(audit_path.read_text(encoding="utf-8"))
    if freeze.get("affinity_feature_dim") != 24 or freeze.get("cohort") != {"train": 2981, "validation": 371} or audit.get("physical_cache_records_verified") != 3352:
        raise RuntimeError("R2 cache contract mismatch")
    for payload in (freeze, audit):
        if payload.get("validation_gt_read") is not False or payload.get("consumer_trained") is not False or payload.get("test_evaluated") is not False:
            raise RuntimeError("R2 cache safety mismatch")
...
def audit_output(source_hashes: dict[str, str], cache: dict[str, object], baseline: dict[str, object], t4: dict[str, object]) -> None:
    ...
    for path in (freeze_path, run_path, prediction_path, score_path, OUTPUT / "affinity_residual.pt", OUTPUT / "training_history.json"):
        if not path.is_file():
            raise RuntimeError(f"R2 output missing: {path}")
    freeze = json.loads(freeze_path.read_text(encoding="utf-8"))
    run_manifest = json.loads(run_path.read_text(encoding="utf-8"))
    if freeze.get("protocol_sha256") != PROTOCOL_SHA256 or freeze.get("validation_predictions") != 371 or freeze.get("affinity_dim") != 24 or freeze.get("training_labels") != "image_level_only" or run_manifest.get("validated_cache_records") != {"train": 2981, "validation": 371}:
        raise RuntimeError("R2 freeze/run contract mismatch")
    for payload in (freeze, run_manifest):
        if payload.get("validation_gt_read") is not False or payload.get("consumer_trained") is not False or payload.get("test_evaluated") is not False:
            raise RuntimeError("R2 output safety mismatch")
    ...
    if len(predictions) != 371 or len(scores) != 371 or len({row["image_id"] for row in predictions}) != 371 or len({row["image_id"] for row in scores}) != 371:
        raise RuntimeError("R2 output cohort mismatch")
    for row in predictions:
        ...
        if hash_file(path) != row["map_sha256"] or values.shape != (320, 320) or values.dtype != np.float16 or not np.isfinite(values).all():
            raise RuntimeError(f"R2 prediction map mismatch: {row['image_id']}")
    for row in scores:
...
def main() -> None:
    ...
    run([sys.executable, "-m", "pytest", "-q", "tests/test_run_mask_bag_affinity_residual_arm.py", "tests/test_mask_bag_affinity_features.py", "tests/test_mask_bag_affinity_residual_training.py", "tests/test_mask_bag_selector_cache.py", "tests/test_mask_bag_selector_cache_io.py"], cwd=SOURCE)
    run([sys.executable, "-m", "pytest", "-q"], cwd=SOURCE)
    run([sys.executable, str(SOURCE / "project/run_mask_bag_affinity_residual_arm.py"), "--split-manifest", str(split), "--expected-split-sha256", SPLIT_SHA256, "--selector-cache-root", str(cache_root), "--expected-selector-cache-freeze-sha256", CACHE_FREEZE_SHA256, "--baseline-root", str(baseline_root), "--expected-baseline-checkpoint-sha256", BASELINE["checkpoint"], "--expected-baseline-source-commit", BASELINE["source_commit"], "--expected-baseline-protocol-sha256", BASELINE["protocol"], "--source-commit", SOURCE_COMMIT, "--protocol-sha256", PROTOCOL_SHA256, "--output-dir", str(OUTPUT)], cwd=SOURCE)
