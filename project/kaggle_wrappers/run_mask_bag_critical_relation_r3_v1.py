...
"""Fail-closed Kaggle bootstrap for the predeclared R3 relation arm."""
...
KERNEL = "itsthang333/btxrd-rad-dino-mask-bag-critical-relation-r3-v1"
KERNEL_VERSION = 0
...
SOURCE = WORK / "r3_source"
RUNTIME = WORK / "r3_runtime"
OUTPUT = WORK / "btxrd_mask_bag_critical_relation_r3_v1"
...
def clone_and_verify() -> dict[str, str]:
    ...
    if hash_file(protocol_path) != PROTOCOL_SHA256:
        raise RuntimeError("R3 protocol hash mismatch")
    protocol = json.loads(protocol_path.read_text(encoding="utf-8"))
    hashes = protocol.get("canonical_lf_source_hashes", {})
    for relative, expected in hashes.items():
        path = SOURCE / relative
        if not path.is_file() or canonical_hash(path) != expected:
            raise RuntimeError(f"R3 source hash mismatch: {relative}")
    return hashes


def install_runtime() -> None:
    run(
        [sys.executable, "-m", "pip", "install", "--disable-pip-version-check", "--no-input", "--no-cache-dir", "--force-reinstall", "--no-deps", "torch==2.5.1+cu121", "torchvision==0.20.1+cu121", "--index-url", "https://download.pytorch.org/whl/cu121"],
        cwd=SOURCE,
    )
    run(
        [sys.executable, "-m", "pip", "install", "--disable-pip-version-check", "--no-input", "--no-cache-dir", "transformers==4.50.2"],
        cwd=SOURCE,
    )
...
def prepare_split() -> Path:
    ...
    if hash_file(source) != GIT_SPLIT_SHA256 or b"\r" in source.read_bytes():
        raise RuntimeError("R3 canonical split mismatch")
    target = RUNTIME / "frozen_split_manifest.csv"
    target.write_bytes(source.read_bytes().replace(b"\n", b"\r\n"))
    if hash_file(target) != SPLIT_SHA256:
        raise RuntimeError("R3 frozen split reconstruction mismatch")
    return target
...
def prepare_baseline() -> tuple[Path, dict[str, object]]:
    ...
    if (
        ...
        raise RuntimeError("R3 baseline transport safety mismatch")
    ...
    if len(set(roots)) != 1:
        raise RuntimeError(f"Expected one R3 baseline root, found {roots}")
    return roots[0], {
...
def find_cache() -> tuple[Path, dict[str, object]]:
    ...
    if hash_file(audit_path) != CACHE_WRAPPER_AUDIT_SHA256:
        raise RuntimeError("R3 cache audit hash mismatch")
    ...
    if (
        ...
        raise RuntimeError("R3 cache contract mismatch")
    for payload in (freeze, audit):
        if (
            ...
            raise RuntimeError("R3 cache safety mismatch")
    return root, {
...
def audit_output(
    ...
    required = (
        ...
        score_path,
        diagnostic_path,
        OUTPUT / "critical_relation_residual.pt",
        OUTPUT / "training_history.json",
        OUTPUT / "pretraining_identity_audit.json",
    )
    for path in required:
        if not path.is_file():
            raise RuntimeError(f"R3 output missing: {path}")
    freeze = json.loads(freeze_path.read_text(encoding="utf-8"))
    ...
    if (
        ...
        raise RuntimeError("R3 freeze/run contract mismatch")
    for payload in (freeze, run_manifest):
        if (
            ...
            raise RuntimeError("R3 output safety mismatch")
    ...
    for rows in (predictions, scores, diagnostics):
        if len(rows) != 371 or len({row["image_id"] for row in rows}) != 371:
            raise RuntimeError("R3 output cohort mismatch")
    for row in predictions:
        ...
        if (
            ...
            raise RuntimeError(f"R3 prediction map mismatch: {row['image_id']}")
    for row in scores:
        ...
        if (
            ...
            raise RuntimeError(f"R3 candidate-score mismatch: {row['image_id']}")
    wrapper_audit = {
        "kernel": KERNEL,
...
def main() -> None:
    ...
    run(
        [sys.executable, "-m", "pytest", "-q", "tests/test_run_mask_bag_critical_relation_arm.py", "tests/test_mask_bag_critical_relation_training.py", "tests/test_mask_bag_relational_selector.py", "tests/test_mask_bag_selector_cache.py", "tests/test_mask_bag_selector_cache_io.py"],
        cwd=SOURCE,
    )
    ...
    run(
        [
            sys.executable,
            str(SOURCE / "project/run_mask_bag_critical_relation_arm.py"),
            "--split-manifest",
            str(split),
