"""Fail-closed Kaggle bootstrap for the S9 candidate-marginal selector."""
...
SOURCE = WORK / "s9_source"
RUNTIME = WORK / "s9_runtime"
OUTPUT = WORK / "btxrd_skelex_candidate_marginal_s9_v1"
...
def clone_and_verify() -> tuple[dict[str, str], dict[str, object]]:
    if not LAUNCH_BINDING_READY or KERNEL_VERSION < 1:
        raise RuntimeError("S9 launch binding is not frozen")
    if len(CHECKOUT_COMMIT) != 40 or any(c not in "0123456789abcdef" for c in CHECKOUT_COMMIT):
        raise RuntimeError("S9 checkout commit is invalid")
    ...
    if (
        ...
        or protocol.get("representation", {}).get("trainable_parameters") != 524801
        ...
        raise RuntimeError("S9 protocol safety/provenance mismatch")
...
def verify_t4x2() -> dict[str, object]:
    ...
    if not all("T4" in name for name in names):
        raise RuntimeError(f"S9 requires T4 x2, got {names}")
    checksums: list[float] = []
    for index in range(2):
        torch.manual_seed(590 + index)
        layer = torch.nn.Conv2d(3, 5, 3, padding=1).to(f"cuda:{index}").eval()
        values = torch.arange(3072, dtype=torch.float32, device=f"cuda:{index}").reshape(
            1, 3, 32, 32
        )
        with torch.inference_mode():
            result = layer(values)
        if not torch.isfinite(result).all():
            raise RuntimeError(f"S9 non-finite T4 guard on cuda:{index}")
...
def download_skelex() -> tuple[Path, dict[str, object]]:
    ...
    for name, expected in SKELEX_FILES.items():
        ...
        if hash_file(target) != expected:
            raise RuntimeError(f"S9 public model hash mismatch: {name}")
    if (root / "model.safetensors").stat().st_size != SKELEX_WEIGHT_BYTES:
        raise RuntimeError("S9 public model size mismatch")
    return root, {
...
def run_static_tests() -> None:
    run(
        [
            ...
            "-m",
            "py_compile",
            "project/models/skelex_candidate_marginal.py",
            "project/run_skelex_candidate_marginal_s9.py",
            "project/audit_skelex_candidate_marginal_s9_output.py",
    ...
    run(
        [
            ...
            "pytest",
            "-q",
            "tests/test_skelex_candidate_marginal.py",
...
def prepare_split() -> Path:
    ...
    if hash_file(source) != GIT_SPLIT_SHA256 or b"\r" in source.read_bytes():
        raise RuntimeError("S9 canonical split mismatch")
    target = RUNTIME / "frozen_split_manifest.csv"
    target.write_bytes(source.read_bytes().replace(b"\n", b"\r\n"))
    if hash_file(target) != SPLIT_SHA256:
        raise RuntimeError("S9 frozen split reconstruction mismatch")
    return target
...
def find_dataset_root() -> Path:
    ...
    if len(valid) != 1:
        raise RuntimeError(f"Expected one S9 BTXRD root, found {sorted(map(str, valid))}")
...
def find_candidate_root(
    *, manifest_sha: str, pseudo_sha: str, expected_split: str
    ...
    if len(set(roots)) != 1:
        raise RuntimeError(f"Expected one S9 {expected_split} candidate root, found {roots}")
    return roots[0], {
...
def find_baseline() -> tuple[Path, dict[str, object]]:
    ...
    for freeze in INPUT.rglob("prediction_freeze.json"):
        ...
        if (
            ...
            roots.append(root.resolve())
    if len(set(roots)) != 1:
        raise RuntimeError(f"Expected one S9 baseline root, found {roots}")
...
def find_cache() -> tuple[Path, dict[str, object]]:
    ...
    for freeze in INPUT.rglob("selector_cache_freeze.json"):
        ...
        if (
            ...
            roots.append(root.resolve())
    if len(set(roots)) != 1:
        raise RuntimeError(f"Expected one S9 selector-cache root, found {roots}")
    root = roots[0]
    ...
    for payload in (freeze, audit):
        if (
            ...
            raise RuntimeError("S9 selector-cache safety mismatch")
    if freeze.get("cohort") != {"train": 2981, "validation": 371}:
        raise RuntimeError("S9 selector-cache cohort mismatch")
    return root, {
...
def _rows(path: Path, expected: int) -> list[dict[str, str]]:
    ...
    if len(rows) != expected or len({row["image_id"] for row in rows}) != expected:
        raise RuntimeError(f"S9 output cohort mismatch: {path}")
    return rows
...
def audit_wrapper_output(
    *,
    ...
    if (
        ...
        or independent.get("feature_hashes_verified") != 3352
        or independent.get("physical_likelihood_evidence_verified") != 371
        or independent.get("physical_prediction_maps_verified") != 742
    ...
    for payload in (pair, independent, binding, run_manifest):
        if (
            ...
            raise RuntimeError("S9 output safety mismatch")
    physical_maps = 0
    physical_scores = 0
    for arm in ARMS:
        freeze_path = OUTPUT / arm / "prediction_freeze.json"
        if hash_file(freeze_path) != pair["arms"][arm]:
            raise RuntimeError(f"S9 arm freeze mismatch: {arm}")
        for row in _rows(OUTPUT / arm / "predictions" / "prediction_manifest.csv", 371):
            ...
            if not path.is_file() or hash_file(path) != row["map_sha256"]:
                raise RuntimeError(f"S9 map hash mismatch: {arm}/{row['image_id']}")
            physical_maps += 1
        for row in _rows(OUTPUT / arm / "candidate_scores" / "candidate_score_manifest.csv", 371):
            ...
            if not path.is_file() or hash_file(path) != row["score_sha256"]:
                raise RuntimeError(f"S9 score hash mismatch: {arm}/{row['image_id']}")
            physical_scores += 1
    evidence_rows = _rows(OUTPUT / "s9_likelihood_evidence" / "evidence_manifest.csv", 371)
    for row in evidence_rows:
        path = OUTPUT / "s9_likelihood_evidence" / row["evidence_path"]
        if not path.is_file() or hash_file(path) != row["evidence_sha256"]:
            raise RuntimeError(f"S9 evidence hash mismatch: {row['image_id']}")
    if physical_maps != 742 or physical_scores != 742:
        raise RuntimeError("S9 wrapper physical output count mismatch")
    wrapper_audit = {
        "kernel": KERNEL,
        "kernel_version": KERNEL_VERSION,
        "bound_wrapper_sha256": canonical_hash(Path(__file__)),
    ...
    path = OUTPUT / "wrapper_output_audit.json"
    with path.open("x", encoding="utf-8", newline="\n") as handle:
...
def main() -> None:
    ...
    try:
        ...
        run(
            [
                sys.executable,
                str(SOURCE / "project/run_skelex_candidate_marginal_s9.py"),
        ...
        run(
            [
                ...
                "--split-manifest", str(split),
                "--skelex-model-dir", str(skelex_root),
                "--skelex-config-sha256", SKELEX_FILES["config.json"],
                "--skelex-preprocessor-sha256", SKELEX_FILES["preprocessor_config.json"],
                "--train-candidate-root", str(train_root),
                "--val-candidate-root", str(val_root),
                "--selector-cache-root", str(cache_root),
                "--baseline-root", str(baseline_root),
