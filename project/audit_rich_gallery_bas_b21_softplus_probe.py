...
"""Independent output audit for the bounded BAS-B2.1 Softplus probe."""
...
from run_rich_gallery_bas_b21_softplus_probe import _mechanics_gate
...
def main() -> None:
    ...
    if (
        summary.get("stage") != "rich_gallery_bas_b21_softplus_mechanics_probe_v1"
        ...
        or summary.get("scientific_delta_from_b2")
        != "terminal_relu_to_softplus_only"
        ...
        raise ValueError("BAS Softplus probe summary contract mismatch")
    if (
        freeze.get("stage")
        != "rich_gallery_bas_b21_softplus_mechanics_probe_freeze_v1"
        ...
        raise ValueError("BAS Softplus probe freeze contract mismatch")

    history_path = args.output_root / "training_history.csv"
    predictions_path = args.output_root / "validation_predictions.csv"
    activation_manifest_path = args.output_root / "activation_manifest.csv"
    checkpoint_path = args.output_root / "bas_b21_softplus_probe_fp16.pt"
    expected_hashes = {
        history_path: summary["training_history_sha256"],
        predictions_path: summary["validation_predictions_sha256"],
        activation_manifest_path: summary["activation_manifest_sha256"],
        checkpoint_path: summary["checkpoint_sha256"],
    }
    for path, expected in expected_hashes.items():
        if sha256_file(path) != expected:
            raise ValueError(f"BAS Softplus probe artifact changed: {path}")

    history = _rows(history_path)
    if len(history) != 5 or [int(row["epoch"]) for row in history] != list(range(1, 6)):
        raise ValueError("BAS Softplus probe training history mismatch")
    predictions = _rows(predictions_path)
    if len(predictions) != 371 or len({row["image_id"] for row in predictions}) != 371:
        raise ValueError("BAS Softplus prediction cohort mismatch")
    labels = np.asarray([int(row["tumor"]) for row in predictions], dtype=np.int64)
    probabilities = np.asarray(
        [float(row["tumor_probability"]) for row in predictions],
        dtype=np.float64,
    )
    ranges = np.asarray(
        [float(row["activation_range"]) for row in predictions],
        dtype=np.float64,
    )
    if int(labels.sum()) != 184 or not np.isfinite(probabilities).all():
        raise ValueError("BAS Softplus prediction labels/probabilities mismatch")
    ...
    for key, value in recomputed_validation.items():
        if not np.isclose(
            value,
            float(summary["validation_diagnostics"][key]),
            rtol=0.0,
            atol=1.0e-12,
        ):
            raise ValueError(f"BAS Softplus validation diagnostic changed: {key}")

    activation_rows = _rows(activation_manifest_path)
    if len(activation_rows) != 371 or len({row["image_id"] for row in activation_rows}) != 371:
        raise ValueError("BAS Softplus activation cohort mismatch")
    for row in activation_rows:
        path = args.output_root / row["activation_path"]
        if sha256_file(path) != row["activation_sha256"]:
            raise ValueError(f"BAS Softplus activation changed: {row['image_id']}")
        array = np.load(path, allow_pickle=False)
        if array.dtype != np.float32 or array.ndim != 2 or not np.isfinite(array).all():
            raise ValueError(f"invalid BAS Softplus activation: {row['image_id']}")
    tumor_area = [
    ...
    if len(tumor_area) != 184:
        raise ValueError("BAS Softplus tumor area-correlation cohort mismatch")
    recomputed_area = {
    ...
    for key, value in recomputed_area.items():
        if not np.isclose(
            value,
            float(summary["candidate_area_diagnostics"][key]),
            rtol=0.0,
            atol=1.0e-12,
        ):
            raise ValueError(f"BAS Softplus area diagnostic changed: {key}")

    gate = _mechanics_gate(history, recomputed_validation, recomputed_area)
    if gate != summary["mechanics_gate"] or bool(gate["pass"]) != bool(
        summary["full_training_authorized"]
    ):
        raise ValueError("BAS Softplus mechanics gate does not reproduce")
    ...
    if (
        ...
        raise ValueError("BAS Softplus checkpoint contract mismatch")
    ...
    if args.audit_output.exists():
        raise FileExistsError("BAS Softplus audit output already exists")
