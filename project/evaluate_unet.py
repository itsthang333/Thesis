...
def main() -> None:
    ...
    # This checkpoint is produced by our audited training process and contains
    # optimizer/NumPy provenance in addition to tensors. PyTorch 2.6 changed
    # the default to weights_only=True, which rejects that trusted metadata.
    ...
    model_architecture = architecture_name_from_metadata(
        checkpoint.get("architecture")
    )
    model = build_segmentation_model(model_architecture, pretrained=False)
    ...
    summary = {
        ...
        "validation_gt_used_by_evaluator": args.split == "val",
