...
"""Run the isolated R4 orbit-averaged critical-relation selector arm."""
...
def main() -> None:
    ...
    if (
        ...
        raise ValueError("R4 execution differs from the frozen finite contract")
    os.environ["CUBLAS_WORKSPACE_CONFIG"] = ":4096:8"
    torch.backends.cudnn.benchmark = False
    torch.backends.cudnn.deterministic = True
    torch.use_deterministic_algorithms(True)
    if not torch.cuda.is_available() or torch.cuda.device_count() != 2:
        raise RuntimeError("R4 requires exactly two visible CUDA devices")
    device_names = [torch.cuda.get_device_name(index) for index in range(2)]
    if not all("T4" in name for name in device_names):
        raise RuntimeError(f"R4 requires Tesla T4 x2, got {device_names}")
    ...
    initial_state = initial_critical_relation_state(
        descriptor_dim=descriptor_dim, hidden_dim=args.hidden_dim, seed=args.seed
    )
    initial_adapter = CriticalRelationResidual(descriptor_dim, args.hidden_dim).to(training_device)
    initial_adapter.load_state_dict(initial_state, strict=True)
    initial_audit = {
        "train": audit_orbit_initialization_records(
            train_records,
            base,
            initial_adapter,
            batch_size=args.batch_size,
        ...
        "validation": audit_orbit_initialization_records(
            val_records,
            base,
            initial_adapter,
            batch_size=args.batch_size,
    ...
    adapter_state = {key: value.detach().cpu().clone() for key, value in adapter.state_dict().items()}
    del base, adapter
    torch.cuda.empty_cache()
    ...
    checkpoint_path = args.output_dir / "orbit_relation_residual.pt"
    torch.save(
        {
    ...
    if len(unordered_scored) != 371 or len(by_image) != 371:
        raise RuntimeError("R4 T4x2 scoring does not cover 371 unique images")
    ...
    swap_exact_count = sum(bool(item["view_swap_exact"]) for item in scored_val)
    gate = {
        ...
        "count_probability_gate_pass": count_spearman <= args.count_probability_spearman_ceiling,
        "view_swap_exact_records": swap_exact_count,
        "view_swap_expected_records": 371,
        "view_swap_gate_pass": swap_exact_count == 371,
    }
    gate["gt_blind_gate_pass"] = bool(
        gate["count_probability_gate_pass"] and gate["view_swap_gate_pass"]
    )
    ...
    run_manifest = {
        "run_id": "btxrd_mask_bag_orbit_relation_r4_v1",
        "source_commit": args.source_commit,
        "protocol_sha256": args.protocol_sha256,
