...
Status: main normalized for thesis writing (2026-07-24). The official validation-frozen WSSS pipeline is `btxrd_wsss_segmenter_v1` at threshold 0.85 (mean tumor Dice 0.230020). The clean test partition remains locked.

Academic classification: `fs_resnet18_pw10_full_448_e20` (Dice 0.495132) uses train ground-truth masks and is retained only as a fully supervised upper-bound diagnostic. It is not the official WSSS pipeline. Rejected causal-selector code and expanded research evidence are preserved on `codex/research-wsss-improvement`, not on the main execution path.
