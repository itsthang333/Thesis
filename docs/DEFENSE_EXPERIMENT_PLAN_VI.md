...
| E1 | Binary hay 10 lớp? | 3 seed binary vs 3 seed 10-class, DenseNet121/320 cùng budget; sau đó cùng downstream | classification metrics và actual mask Dice | Classification hoàn tất: 10-class có xu hướng tốt hơn nhưng paired CI cắt 0. Hai downstream đang/chờ chạy; chưa được kết luận binary tốt hơn. |
| E2 | Vì sao LayerCAM? Vì sao prompt này? | CAM, Grad-CAM, Grad-CAM++, LayerCAM x point/box/box+point | selected Dice, CAM-only p90 Dice, proposal oracle | 12 selected-mask arms hoàn tất. LayerCAM-point tốt nhất 0.205224; LayerCAM marginal 0.193677, hơn ba attribution còn lại với paired CI không cắt 0. Prompt point hơn nhưng CI của prompt main effect còn cắt 0. Cần tải đủ candidate để tách attribution khỏi SAM proposal. |
| E3 | Vì sao SAM ViT-B? | ViT-B/L/H giữ nguyên source maps, prompts, gallery, selector | Dice, oracle, subgroup, runtime, peak VRAM, disk | B/L hoàn tất đang tải/audit; H đang chạy. Không được kết luận ViT-B trước kết quả này. |
...
| E6 | Vì sao G1 và công thức/loss đó? | random, SAM-IoU, upstream, G1, fusion, oracle; sau đó feature/loss ablation | Dice, regret, source error, 3-seed stability | Selector controls hoàn tất: native Dice 0.101890/0.098902/0.225306/0.205545/0.288224; oracle 0.527902. G1 một mình không tốt hơn upstream; chỉ có bằng chứng về tính bổ sung khi fusion. Feature/loss ablation còn thiếu. |
| E7 | Upstream score từ đâu? | U0 SAM; U1 D; U2 M; U3 D+M; U4 D+M+local-R equal; U5 0.60/0.25/0.15; U6 global-R; mỗi arm upstream-only và +G1 | actual Dice + component evidence | Code source-correct đã triển khai. Bắt buộc dùng saliency đúng nguồn và group `(source,component)`; chưa có kết quả cuối. |
