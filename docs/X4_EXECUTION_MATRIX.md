...
|---|---|---|---|
| `wanwin` | `btxrd-x4-s2c-mask-freeze` | W2 audited train/validation pseudo-mask freeze | Running |
...
| W2 | S2C-to-U-Net | Canonical train/validation S2C cache adoption audits pass. Fixed 20-epoch generator checkpoint `b46ac9af...05f3` and its training output passed independent audit. The checkpoint+validation-cache dataset passed 375/375 download-back equality, and the bounded train/validation mask-freeze kernel is running. A staged CPU-only target-freeze kernel validates the upstream receipt/archive and every one of the 2,981 native masks with the exact student-side loader before producing the common bundle; focused regression is 4/4 pass. | Running | When mask-freeze completes, launch the CPU target-freeze chain immediately, then package and train/evaluate three S2C seeds. |
| W3 | Rich-Gallery+G1+R7-to-U-Net | Direct frozen Rich Gallery/G1/R7 choices and candidate diagnostics exist. The common target freezer accepts `rich_gallery`. `project/run_x4_rich_gallery_train_score_kaggle.py` reuses the exact Geometry-v3 train candidate bank and locked G1 checkpoint to freeze choices for all 2,981 canonical train images without spatial GT/test access. The private/offline T4 train-score kernel is staged. A separate staged CPU-only `project/run_x4_rich_gallery_target_freeze_kaggle.py` consumes that immutable output, builds all 2,981 native masks and validates them with the exact student-side fail-closed loader. The combined wrapper/target regression is 5/5 pass. | Implemented and staged, not launched | Launch the bounded train-score job in the first appropriate `wanwin` GPU slot, immediately chain the CPU target-freeze job, then train/evaluate three matched students. |
...
| X7 | Seeds 42/43/44 for CAM, PuzzleCAM, S2C, Rich Gallery and fully supervised | Seed contract is frozen and common trainer supports all arms/seeds. CAM 42/43/44 and PuzzleCAM 42/43 are running; the three fully-supervised kernels are staged and pass filename/arm/provenance checks. | Running | Complete 15 matched student runs and report mean plus sample SD. |
