...
| `wanwin` | `btxrd-x4-yolov8s-seg-seed42` v1 | X3 retry YOLOv8s-seg seed42 on a different account/worker | Running |
...
| `itsthang333` | `btxrd-x4-yolov8s-seg-seed43` v1 | X3 YOLOv8s-seg seed43 | Running |
...
| X3 | YOLOv8s-seg upper bound, 600 px, 300 epochs, native mAP and common binary-union evaluation | `project/export_x4_yolo_dataset.py` exports the exact canonical train/val polygons in Ultralytics instance-seg format; `project/train_x4_yolov8s_seg.py` fixes official YOLOv8s-seg, Ultralytics 8.4.0, 600 px and 300 epochs; Stage A freezes native union masks and the separate evaluator reports native mAP plus all common X4 metrics. Seed-42 v1/v2 on `itsthang333` failed before a worker log. The exact offline payload was therefore replicated to `wanwin`; seed42 now runs there, while seed43 runs on `itsthang333`. | Running seeds 42/43 | Distinguish account/worker startup failure from model runtime, then audit common outputs and add seed44. |
...
| X7 | Seeds 42/43/44 for CAM, PuzzleCAM, S2C, Rich Gallery and fully supervised | All 15 required trainings are now either complete or running: CAM 3/3 evaluated; PuzzleCAM 3/3 trained and 2/3 evaluated; Fully 3/3 trained; S2C 2/3 trained plus seed43 running; Rich 3/3 running. | Running/evaluation | Finish artifact audits and 371-image evaluations, then report mean plus sample SD. |
