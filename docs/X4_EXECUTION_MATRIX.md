...
| X10 | Protocol-selected qualitative panel | `project/select_x4_qualitative_cases.py` freezes cases before image/GT rendering using Dice quantiles, size groups and deterministic failure-category rules. The direct baseline freeze supplies 11/12 categories without visual cherry-picking. | Partial | Add the normal-FP case after predicted-gate/student evaluation, then render the full image/evidence/candidate/student/fully/GT panel. |
