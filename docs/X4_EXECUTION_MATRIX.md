...
| `itsthang333` | `btxrd-x4-yolov8s-seg-seed42` v2 | X3 polygon-supervised YOLOv8s-seg upper bound, seed 42 | Running after one pre-worker retry |
...
| W0 | CAM-to-U-Net | CAM train/validation mask freezes and audits are complete. The standardized 2,981-image target bundle is frozen. Seeds 42, 43 and 44 completed; seed 43 training audit passed with checkpoint `cd3e4d82...0cb8`, best epoch 8 and inner-only threshold 0.65. | Training complete; audits/predictions pending | Audit seeds 42/44, then freeze/evaluate 371 predictions for all seeds. |
| W1 | PuzzleCAM-to-U-Net | PuzzleCAM generator training and train/validation mask audits are complete. The standardized target bundle is frozen. Seeds 42 and 43 completed; seed 44 is running. | Running | Download/audit seeds 42/43, then freeze/evaluate all completed predictions. |
...
| X7 | Seeds 42/43/44 for CAM, PuzzleCAM, S2C, Rich Gallery and fully supervised | Seed contract is frozen and common trainer supports all arms/seeds. CAM 42/43/44 and PuzzleCAM 42/43 completed; PuzzleCAM44 runs. Fully-supervised 42/43/44 run after explicit per-payload seed binding fixed Kaggle's `script.py` rename. | Running | Complete 15 matched student runs and report mean plus sample SD. |
