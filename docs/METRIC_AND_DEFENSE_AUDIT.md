...
| E2 | Why LayerCAM and which prompt? | 4 attribution methods x point/box/point+box; CAM-only threshold control | Classifier, source, SAM-B, candidate/evaluator protocol | Actual Dice/oracle/subgroups, paired main effects, runtime | Spatial decomposition complete: LayerCAM has supported positive main effects; point is numerically best but prompt contrasts cross zero; LayerCAM+point selected/CAM-only/oracle Dice is 0.205224/0.152326/0.339441. Per-arm resource telemetry remains pending, so E2 is not yet fully closed. |
