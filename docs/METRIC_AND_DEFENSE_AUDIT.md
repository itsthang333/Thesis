...
| E1 | Why binary rather than 10-class supervision? | B2 versus matched C10, seeds 42/43/44; collapse C10 by exact tumor log-odds; then identical downstream pipeline | Backbone, optimizer, epochs, checkpoint endpoint, split, SAM-B, gallery, G1, fusion | Image discrimination/calibration **and downstream selected/oracle/CAM-only Dice** | Complete/audited. C10 is better calibrated and has downstream Dice 0.298954 versus 0.262899 for B2; CAM-only Dice is 0.172241 versus 0.133468. The gain is medium/large dominated and does not solve small lesions. Binary superiority is rejected under this matched protocol. |
| E2 | Why LayerCAM and which prompt? | 4 attribution methods x point/box/point+box; CAM-only threshold control | Classifier, source, SAM-B, candidate/evaluator protocol | Actual Dice/oracle/subgroups, paired main effects, runtime | Complete. LayerCAM has supported positive main effects; point is numerically best but prompt contrasts cross zero; LayerCAM+point selected/CAM-only/oracle Dice is 0.205224/0.152326/0.339441. Immutable Kaggle T4 logs give 262.901--342.988 s generation per 371-image arm and bind every arm start/freeze pair. |
| E3 | Why SAM ViT-B? | ViT-B/L/H end-to-end | Inputs, prompts, multimask, gallery merge/dedup/cap, G1, fusion | Selected/oracle Dice, subgroups, seconds/image, peak VRAM, disk | Complete/audited. B/L/H Dice is 0.288729/0.291185/0.279212 and oracle is 0.528298/0.546000/0.510446. L-B delta +0.002456 has CI `[-0.020021,0.024824]` at 1.84x total time. H-B delta -0.009517 has CI `[-0.034714,0.015280]` at 3.17x total time and 2.02x peak allocated VRAM. |
| E4 | Why three localization sources? | All seven non-empty subsets of LayerCAM-320, LayerCAM-448 and external saliency | Exact selector, cap policy and cohort | Candidate count/oracle/Recall@Dice, selected Dice, regret, runtime/storage | Complete/audited on the current split. Full selected/oracle Dice is 0.288729/0.528298; no pair reaches full selected Dice and no single source exceeds 0.275499. All-seven replay time is 47.524 s after reusing the 1615.226 s frozen gallery. Complementarity is supported overall, not claimed per image/subgroup. |
| E5 | Why a rich gallery and why cap 243? | Upstream top-1; one exact prompt; three multimasks; full pre-dedup; post-dedup; caps 27/81/162/243 | Source generation and selector | Oracle@stage/K, selected Dice, truncation regret, candidate count, resource curve | Complete/audited. Native Dice is 0.222746/0.219017/0.219600/0.288139/0.288224/0.288224 for upstream top-1, exact single-mask, exact three-multimask, pre-dedup, post-dedup and cap243. The full gallery adds about 0.0686 Dice over one prompt; three masks alone add only 0.00058 over one mask. Dedup is negligible (+0.000085), while cap243 is inactive for these already-bounded banks. |
...
| E6b | Which G1 features/losses are necessary? | Four cumulative feature arms and four cumulative loss arms, 3 seeds, matched capacity | Cached RAD-DINO descriptors, scorer size, train budget, split | Selected Dice/oracle/regret plus AUROC/AUPRC/calibration and seed variation | Complete/audited. Full feature/loss is the best three-seed mean at native Dice `0.279855 +/- 0.007371`, still below fixed R7 `0.288224`. Bag-negative seed 42 reaches `0.290598`, but the family mean is `0.279598 +/- 0.010227`; it cannot be selected post hoc. The winner and consistency terms are therefore not established as necessary improvements. |
| E7 | Why `0.60D+0.25M+0.15R`? | U0--U6: raw SAM, D, M, equal combinations, current local rank, global-rank variant | Candidates, G1, fusion/evaluator | Actual Dice, paired CI, subgroup/regret | Complete/audited. Legacy U5+R7 native Dice 0.288224, source-correct U5+R7 0.289358, and source-correct U6+R7 0.294956. The U6 delta +0.006732 has CI `[-0.010248,0.024597]`; therefore the coefficients remain frozen empirical choices, not literature constants or statistically proven optima. |
...
E1--E8, including exact E5, now have frozen validation evidence and independent
audits. The defensible wording is: the current final system is the best
**validation-selected configuration among the completed declared arms**. E5
supports the necessity of a diverse prompt/source gallery but does not support
a claim that three SAM multimasks, exact deduplication or the number 243 is a
universal optimum. E6b specifically shows that no declared retrained G1 family
improves the fixed baseline on its predeclared three-seed mean.
